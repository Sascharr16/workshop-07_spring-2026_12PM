...
H~0~: **pH does not predict abalone shell growth**  

H~A~: **pH does predict abalone shell growth**
...
abalone_clean <- abalone |>  
  # cleaning column titles to remove spaces
  clean_names()  |> 
  # selecting pH and change in area columns
  select(mean_p_h, change_in_area_mm2_d_1_25)  |> 
  # renaming mean pH title and change in area title
...
# base layer: ggplot
ggplot(data = abalone_clean, #  data: abalone clean
       aes(x = mean_ph, # x-axis is ph, y-axis is change in shell surface area
           y = change_in_area)) +
  # first layer: geom_point
  # creating visual of observations points
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**missing value comes from one pair of tanks in which the pH sensor malfunctioned**  
...
abalone_model <- lm(
  ...
  data = abalone_clean      # data: clean abalone data
)
```
...
**At pH = 8.0, the model predicts change in abalone shell surface area to be 5.36 [95% CI: 5.08, 5.64] (mm^2)**
